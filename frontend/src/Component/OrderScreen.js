...
export default function OrderScreen() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      const loadPaypalScript = async () => {
        const { data: clientId } = await instance.get('/orders/keys/paypal/', {
          headers: { authorization: `Bearer ${userInfo.token}` },
        });
        paypalDispatch({
          type: 'resetOptions',
  ...
  return loading ? (
    ...
    <div>
      ...
      <h1 className="my-3">Thanh toán đơn hàng {orderId}</h1>
      <Row>
        <Col md={8}>
          <Card className="mb-3">
            <Card.Body>
              ...
              {order.isDelivered ? (
                <MessageBox variant="success">
                  Xác nhận vận chuyển giao hàng {order.deliveredAt}
                </MessageBox>
              ) : (
